...
Spectrum & Spectrum::operator=( const Spectrum &i_spectrum )
{
    for ( size_t i = 0; i < SPECTRUM_SAMPLE_COUNT; ++i )
    {
        m_wavelengths[ i ] = i_spectrum.m_wavelengths[ i ];
    }
    
    return *this;
}

Spectrum Spectrum::operator+( const Spectrum &i_spectrum ) const
{
    Spectrum ret = *this;
    
    for ( size_t i = 0; i < SPECTRUM_SAMPLE_COUNT; ++i )
    {
        ret.m_wavelengths[i] += i_spectrum.m_wavelengths[ i ];
    }
...
Spectrum Spectrum::operator+( float i_constant ) const
{
    Spectrum ret = *this;
    
    for ( size_t i = 0; i < SPECTRUM_SAMPLE_COUNT; ++i )
    {
        ret.m_wavelengths[i] += i_constant;
    }
    
    return ret;
}

Spectrum operator+( float i_lhs, const Spectrum &i_rhs )
{
    Spectrum ret;
    
    for ( size_t i = 0; i < SPECTRUM_SAMPLE_COUNT; ++i )
    {
        ret.m_wavelengths[i] = i_lhs + i_rhs.m_wavelengths[ i ];
    }
    
    return ret;
}

Spectrum & Spectrum::operator+=( float i_constant )
{
    for ( size_t i = 0; i < SPECTRUM_SAMPLE_COUNT; ++i )
    {
        m_wavelengths[ i ] += i_constant;
    }
...
Spectrum sqrt( const Spectrum &i_spectrum )
{
    Spectrum ret;
    
    for ( size_t i = 0; i < SPECTRUM_SAMPLE_COUNT; ++i )
    {
        ret.m_wavelengths[ i ] = std::sqrt( i_spectrum.m_wavelengths[ i ] );
    }
